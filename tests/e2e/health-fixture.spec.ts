...
const loadControlledHome = async (page: Page) => {
  await page.route('**/en', async (route) => {
    await route.fulfill({
      body: `<!doctype html><html lang="${englishHome.expectedLanguage}"><head><title>${englishHome.expectedTitle}</title></head><body><main>Fixture page</main><a class="language-switch" aria-current="language" href="${englishHome.path}">English</a></body></html>`,
